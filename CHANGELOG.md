...
- The Go UDS receiver now rejects startup cancellation observed after its
  cancellation watcher and accept loop have both started but before readiness
  returns. Rejection waits for both lifecycle goroutines and any accepted
  handlers to terminate, clears receiver ownership, removes only its captured
  public/private artifacts, and preserves a replacement listener pathname.
...
- The Go UDS receiver now rejects startup cancellation observed after listener
  ...
  and preserves a replacement listener installed at the configured pathname.
- The Go UDS receiver now rejects startup cancellation observed after its
...
- The Go UDS receiver now rejects startup cancellation observed after private
  ...
  detached listener plus its private staging artifacts are removed.
- The Go UDS receiver now probes a pre-existing Unix socket before startup,
  rejects a connectable active listener without replacing its pathname or
...
- The Go UDS receiver now rejects pre-existing regular files and symlinks at
  ...
  is disabled so shutdown removes only the non-following device/inode/change-
  time identity created by that receiver and fails closed when generation
  metadata is missing or changed, preserving a replacement Unix listener,
  regular file, or symlink; existing Unix-socket reclamation behavior remains
  unchanged.
