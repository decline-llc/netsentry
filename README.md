...
- Minimal HTTP endpoints: `/api/health` with verbose component snapshot, paginated and filterable `/api/alerts`, `/api/metrics`, rule listing, rule create/update/delete, rule reload, and optional PSK Bearer auth for modifying rule endpoints.
...
- Suppressions API wiring, advanced alert querying, and remaining error envelope coverage.
