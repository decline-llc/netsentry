...
- Each decoded recovery record must retain the required identity, time, count,
  ...
  plus `dst_ip` must be strict IPv4 addresses. MITRE tactic, technique ID, and
  technique name must be either exactly all empty or all nonblank; complete
  tuple text is preserved without current-catalog revalidation. An altered
  event identity, missing or blank rule name, empty, case-variant, or
  unsupported severity, partial or whitespace-only MITRE tuple, or malformed,
