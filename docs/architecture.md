...
Current build: zap startup and match logs, verbose health, Prometheus metrics for process/queue/rule latency/alert write latency/alert/storage/worker/capture heartbeat state, structured audit logs for non-GET API requests, optional localhost-only pprof, and configurable payload preview redaction before alert writes.

v0.1.0 target:

- `/api/metrics` Prometheus endpoint with process counters, rule match and alert write latency buckets, queue/rule/alert/storage gauges, worker counters, and capture heartbeat gauges.
