...
| R90-15 | Jul 20–Sep 18 | Implemented; delivery pending | Reject incompatible existing SQLite schemas before writable initialization. | R90-14 | A structurally valid but non-NetSentry or incompatible existing database fails startup clearly and remains byte-for-byte unchanged; compatible existing, empty, and missing databases retain current behavior. |
...
- **Current result:** The empty queue was refreshed on Jul 19 from verified
  ...
  release is authorized. R90-15 was selected on Jul 20 from the clean fetched
  `origin/main` baseline and verified R90-14 Vault evidence; its implementation
  and local validation pass, with delivery verification pending.
...
`R90-01 → R90-02 → R90-03`; `R90-03a → R90-04a`; `R90-04 → R90-04b → R90-05 → R90-06 → R90-07 → R90-08 → R90-09 → R90-10 → R90-11 → R90-12 → R90-13 → R90-14 → R90-15`. R90-04a is an evidence-independent quality increment and does not satisfy any R90-04 dependency. The R90-04 and R90-05 PCAP exceptions remain immutable historical delivery evidence. The later global PCAP waiver supersedes their restrictions for current and future release-gate decisions.
