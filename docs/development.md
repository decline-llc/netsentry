...
- Go tests cover receiver frame validation/lifecycle, connection caps and read-idle expiry, worker-pool shutdown, panic isolation, rule/MITRE semantics, API limits, SQLite aggregation, daily shards, recovery-log replay and semantic validation, corrupt/truncated/incompatible-schema startup and historical-shard preservation, active WAL-backed read-only access, and storage degraded/emergency behavior.
