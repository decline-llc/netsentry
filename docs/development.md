...
- Go tests cover receiver frame validation/lifecycle, connection caps and read-idle expiry, worker-pool shutdown, panic isolation, rule/MITRE semantics, API limits, SQLite aggregation, daily shards, bounded recovery-log encoding/replay and semantic validation, corrupt/truncated/write-blocking-schema startup and historical-shard preservation, compatible nullable/defaulted schema extensions, active WAL-backed read-only access, and storage degraded/emergency behavior.
