...
uniqueness contract. It also rejects extra uniqueness constraints, generated
columns, triggers, `CHECK` constraints, and foreign-key relationships that
attach to or reference either write-critical table and can alter or reject
NetSentry's fixed writes. Corrupt, truncated,
...
handle, while the current shard continues to use the store-owned connection.
...
- Go tests cover receiver frame validation/lifecycle, connection caps and read-idle expiry, worker-pool shutdown, panic isolation, rule/MITRE semantics, API limits, SQLite aggregation, daily shards, bounded recovery-log encoding/replay and semantic validation, corrupt/truncated/write-blocking-schema startup and historical-shard preservation, generated-column, `CHECK`-constraint, and write-critical foreign-key rejection, compatible ordinary column/index/unrelated-table extensions, active WAL-backed read-only access, and storage degraded/emergency behavior.
