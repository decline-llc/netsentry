...
| `GET /api/metrics` | partial | Prometheus text output exists for process counters, queue/rule/alert gauges, worker counters, and capture heartbeat gauges; latency distributions and disk-space metrics are still pending. |
